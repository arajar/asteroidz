...
namespace math
{
	...
	template<typename T>
	vec2_type<T> normalize(const vec2_type<T>& v) { return v / length(v); }

	template<typename T>
	vec3_type<T> normalize(const vec3_type<T>& v) { return v / length(v); }

	template<typename T>
	vec4_type<T> normalize(const vec4_type<T>& v) { return v / length(v); }
